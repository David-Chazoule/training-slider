...
function Slider() {
  ...
  const nextSlide = () => {
    if (slideAnim.index !== dataSlider.length && !setSlideAnim.inProgress) {
  ...
  const prevSlide = () => {
    if (slideAnim.index !== 1 && !setSlideAnim.inProgress) {
      setSlideAnim({ index: slideAnim.index - 1, inProgress: true });

      setTimeout(() => {
        setSlideAnim({ index: slideAnim.index - 1, inProgress: false });
      }, 400);
    } else if (slideAnim.index === 1 && !setSlideAnim.inProgress) {
